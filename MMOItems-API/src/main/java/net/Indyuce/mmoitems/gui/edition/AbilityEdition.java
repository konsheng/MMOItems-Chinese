...
public class AbilityEdition extends EditionInventory {
	...
	public Inventory getInventory() {
		...
		abilityItemLore.add(ChatColor.GRAY + "选择您的物品所持有的技能.");
		abilityItemLore.add("");
		abilityItemLore.add(
				ChatColor.GRAY + "当前值: " + (ability == null ? ChatColor.RED + "没有已选择的技能." : ChatColor.GOLD + ability.getName()));
		abilityItemLore.add("");
		abilityItemLore.add(ChatColor.YELLOW + AltChar.listDash + " 左键点击以选择.");
		abilityItemLore.add(ChatColor.YELLOW + AltChar.listDash + " 右键点击以重置.");
		...
		if (ability != null) {
			...
			castModeItemLore.add(ChatColor.GRAY + "选择玩家需要进行的动作");
			castModeItemLore.add(ChatColor.GRAY + "为了施展技能而进行的动作.");
			castModeItemLore.add("");
			castModeItemLore.add(ChatColor.GRAY + "当前值: "
					+ (castMode == null ? ChatColor.RED + "没有已选择的触发器." : ChatColor.GOLD + castMode.getName()));
			castModeItemLore.add("");
			castModeItemLore.add(ChatColor.YELLOW + AltChar.listDash + " 左键点击以选择.");
			castModeItemLore.add(ChatColor.YELLOW + AltChar.listDash + " 右键点击以重置.");
		...
		if (ability != null) {
			...
			for (String modifier : ability.getHandler().getModifiers()) {
				...
				modifierItemLore.add("" + ChatColor.GRAY + ChatColor.ITALIC + "这是一个技能属性. 更改此");
				modifierItemLore.add("" + ChatColor.GRAY + ChatColor.ITALIC + "值将会影响技能的最终效果.");
				...
				try {
					...
					modifierItemLore.add(ChatColor.GRAY + "无法读取值. 将使用默认值");
				}

				modifierItemLore.add(ChatColor.GRAY + "默认值: " + ChatColor.GOLD + MODIFIER_FORMAT.format(ability.getDefaultModifier(modifier)));
				modifierItemLore.add("");
				modifierItemLore.add(ChatColor.YELLOW + AltChar.listDash + " 左键点击以更改.");
				modifierItemLore.add(ChatColor.YELLOW + AltChar.listDash + " 右键点击以重置.");
		...
		glassMeta.setDisplayName(ChatColor.RED + "- 没有技能属性 -");
	...
	public void whenClicked(InventoryClickEvent event) {
		...
		if (item.getItemMeta().getDisplayName().equals(ChatColor.GREEN + "技能")) {
			if (event.getAction() == InventoryAction.PICKUP_ALL)
				new StatEdition(this, ItemStats.ABILITIES, configKey, "ability").enable("请在聊天框内输入您想要的技能的名称.",
						"您可以通过此命令获取完整的技能列表 " + ChatColor.AQUA + "/mi list ability");
			...
			if (event.getAction() == InventoryAction.PICKUP_HALF) {
				if (getEditedSection().contains("ability." + configKey + ".type")) {
					...
					player.sendMessage(MMOItems.plugin.getPrefix() + "技能已成功重置.");
				}
			}
			return;
		}
		...
		if (item.getItemMeta().getDisplayName().equals(ChatColor.GREEN + "触发器")) {
			...
			if (event.getAction() == InventoryAction.PICKUP_HALF && getEditedSection().contains("ability." + configKey + ".mode")) {
				...
				registerTemplateEdition();
				player.sendMessage(MMOItems.plugin.getPrefix() + "技能触发器已成功重置.");
			}
			return;
		}
		...
		if (event.getAction() == InventoryAction.PICKUP_ALL)
			new StatEdition(this, ItemStats.ABILITIES, configKey, tag).enable("请在聊天框中输入您想要的值.");
