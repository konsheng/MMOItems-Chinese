...
public class ArrowParticles extends ItemStat<ArrowParticlesData, ArrowParticlesData> {
	public ArrowParticles() {
		super("ARROW_PARTICLES", VersionMaterial.LIME_STAINED_GLASS.toMaterial(), "箭头粒子",
				new String[] { "弓射出的箭后飞在空中时的", "粒子效果." }, new String[] { "bow", "crossbow" });
	}
	...
	public ArrowParticlesData whenInitialized(Object object) {
		...
		Validate.isTrue(config.contains("particle"), "找不到您所设置的箭头粒子");
	...
	public void whenInput(@NotNull EditionInventory inv, @NotNull String message, Object... info) {
		...
		if (edited.equals("color")) {
			...
			inv.getPlayer().sendMessage(MMOItems.plugin.getPrefix() + "粒子颜色已成功设置成 "
		...
		if (edited.equals("amount")) {
			...
			inv.registerTemplateEdition();
			inv.getPlayer().sendMessage(MMOItems.plugin.getPrefix() + ChatColor.GOLD + "数量" + ChatColor.GRAY + " 已设置为 " + ChatColor.GOLD + value
					+ ChatColor.GRAY + ".");
			return;
		}
		...
		inv.getPlayer().sendMessage(MMOItems.plugin.getPrefix() + ChatColor.GOLD + MMOUtils.caseOnWords(edited.replace("-", " ")) + ChatColor.GRAY
				+ " 已设置为 " + ChatColor.GOLD + value + ChatColor.GRAY + ".");
	}
	...
	public void whenDisplayed(List<String> lore, Optional<ArrowParticlesData> statData) {
		...
		lore.add("");
		lore.add(ChatColor.YELLOW + AltChar.listDash + " ► 点击以编辑.");
	}
