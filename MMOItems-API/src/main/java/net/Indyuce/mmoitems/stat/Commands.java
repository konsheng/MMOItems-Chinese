...
public class Commands extends ItemStat<CommandListData, CommandListData> {
	...
	public Commands() {
		super("COMMANDS", VersionMaterial.COMMAND_BLOCK_MINECART.toMaterial(), "指令",
				new String[] { "你的物品在右键后会执行", "的指令." }, new String[] { "!armor", "!block", "!gem_stone", "all" });
	}
	...
	public void whenInput(@NotNull EditionInventory inv, @NotNull String message, Object... info) {
		if (inv.getEditedSection().contains("commands"))
			if (inv.getEditedSection().getConfigurationSection("commands").getKeys(false).size() >= max) {
				...
				inv.open();
				inv.getPlayer().sendMessage(MMOItems.plugin.getPrefix() + "您的物品已达到 " + max + " 命令限制.");
				return;
			}
		...
		inv.getPlayer().sendMessage(MMOItems.plugin.getPrefix() + "指令已成功注册.");
	}

	@Override
	public void whenDisplayed(List<String> lore, Optional<CommandListData> statData) {
		lore.add(ChatColor.GRAY + "当前指令: " + ChatColor.RED
				+ (statData.isPresent() ? ((CommandListData) statData.get()).getCommands().size() : "0"));
		lore.add("");
		lore.add(ChatColor.YELLOW + AltChar.listDash + "► 点击以编辑物品指令.");
	}
